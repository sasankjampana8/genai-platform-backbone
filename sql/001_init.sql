...
CREATE TABLE IF NOT EXISTS app_documents (
    ...
    user_id UUID NOT NULL REFERENCES app_users(user_id),
    knowledge_base_id UUID NOT NULL REFERENCES app_knowledge_bases(knowledge_base_id),
...
CREATE TABLE IF NOT EXISTS app_processing_jobs (
    processing_job_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES app_users(user_id),
    document_id UUID NOT NULL REFERENCES app_documents(document_id),
    knowledge_base_id UUID NOT NULL REFERENCES app_knowledge_bases(knowledge_base_id),
...
CREATE TABLE IF NOT EXISTS app_chats (
    ...
    user_id UUID NOT NULL REFERENCES app_users(user_id),
    knowledge_base_id UUID REFERENCES app_knowledge_bases(knowledge_base_id),
    title TEXT NOT NULL,
    status TEXT NOT NULL DEFAULT 'active',
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
...
CREATE TABLE IF NOT EXISTS app_messages (
    ...
    chat_id UUID NOT NULL REFERENCES app_chats(chat_id),
    user_id UUID NOT NULL REFERENCES app_users(user_id),
    role TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS app_runs (
    ...
    user_id UUID NOT NULL REFERENCES app_users(user_id),
    chat_id UUID REFERENCES app_chats(chat_id),
    message_id UUID REFERENCES app_messages(message_id),
    status TEXT NOT NULL DEFAULT 'queued',
    route TEXT,
    answer TEXT,
...
CREATE TABLE IF NOT EXISTS app_prompt_versions (
    ...
    prompt_id UUID NOT NULL REFERENCES app_prompts(prompt_id),
    template TEXT NOT NULL,
    variables JSONB NOT NULL DEFAULT '[]'::jsonb,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);

CREATE TABLE IF NOT EXISTS app_evaluation_datasets (
...
CREATE TABLE IF NOT EXISTS app_evaluation_cases (
    evaluation_case_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    dataset_id UUID NOT NULL REFERENCES app_evaluation_datasets(dataset_id),
    query TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS app_evaluation_runs (
